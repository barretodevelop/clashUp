...
import 'package:clashup/services/analytics/interfaces/analytics_interface.dart';
...
class NotificationService {
  ...
  static StreamSubscription<RemoteMessage>?
      _backgroundSubscription; // onMessageOpenedApp
  ...
  static Future<bool> _requestPermissions() async {
    try {
      ...
      final hasPermission = permission.isGranted &&
  ...
  static void _onForegroundMessage(RemoteMessage message) {
    ...
    _receivedNotifications.add({
      'id': message.messageId ?? Random().nextInt(100000).toString(),
    ...
    _notificationsController.add(_receivedNotifications); // Notifica listeners
  ...
  static Future<void> toggleNotifications(bool enable) async {
    ...
    AppLogger.info('📢 Notificações ${enable ? "ativadas" : "desativadas"}');
  }

  /// Stream de notificações recebidas
  static Stream<List<Map<String, dynamic>>> get onNotificationsReceived =>
      _notificationsController.stream;

  /// Obter todas as notificações recebidas
  static List<Map<String, dynamic>> getAllNotifications() =>
      List.from(_receivedNotifications);

  /// Limpar todas as notificações recebidas
  static void clearAllNotifications() => _receivedNotifications.clear();
  ...
  /// Simula o recebimento de notificações para fins de demonstração.
  static void _startNotificationSimulation() {
    if (!_notificationsEnabled) return;

    Timer.periodic(const Duration(seconds: 10), (timer) {
      final messages = [
        {'title': 'Nova Conexão!', 'body': 'Alguém está interessado em você.'},
        {
          'title': 'Desafio Concluído!',
          'body': 'Seu desafio diário foi completado.'
        },
        {
          'title': 'Evento Próximo',
          'body': 'Lembrete: "Encontro de Almas" começa em 1h.'
        },
        {
          'title': 'Mensagem de Amigo',
          'body': 'Você tem uma nova mensagem de um amigo.'
        },
      ];
      final random = Random();
